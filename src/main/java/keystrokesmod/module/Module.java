...
public class Module {
    ...
    /** Whether this module appears in the click GUI module list. */
    public boolean isVisibleInClickGui() {
        if (moduleCategory == category.client
                || moduleCategory == category.configs
                || moduleCategory == category.profiles
                || moduleCategory == category.scripts) {
            return true;
        }
        if (closetModule) {
            return true;
        }
        return "HUD".equals(moduleName)
                || "Target HUD".equals(moduleName)
                || "Silent Aura".equals(moduleName)
                || "Clutch".equals(moduleName);
    }
