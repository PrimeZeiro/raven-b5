...
public class RenderUtils implements IMinecraftInstance {
    ...
    // -------------------------------------------------------------------------
    // Full-colour texture helpers (preserves original ARGB, no white-masking)
    // -------------------------------------------------------------------------
    private static final java.util.Map<String, ResourceLocation> fullColorTextureCache = new java.util.HashMap<>();

    /**
     * Returns a cached full-colour (ARGB-preserving) texture for the given
     * classpath resource path (e.g. "/assets/keystrokesmod/textures/gui/raven_logo.png").
     */
    public static ResourceLocation getFullColorTexture(String resourcePath) {
        ResourceLocation cached = fullColorTextureCache.get(resourcePath);
        if (cached != null) return cached;
        try (InputStream stream = Raven.class.getResourceAsStream(resourcePath)) {
            if (stream == null) return null;
            BufferedImage src = ImageIO.read(stream);
            String registryName = "raven_fullcolor_" + Math.abs(resourcePath.hashCode());
            ResourceLocation loc = mc.getTextureManager().getDynamicTextureLocation(registryName, new DynamicTexture(src));
            fullColorTextureCache.put(resourcePath, loc);
            return loc;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    /**
     * Draws a full-colour texture at (x, y) with the given pixel dimensions and
     * an overall alpha multiplier in [0,1].
     */
    public static void drawFullColorTexture(ResourceLocation texture, float x, float y, float width, float height, float alpha) {
        ...
        GL11.glPushMatrix();
        GL11.glTranslatef(x, y, 0f);
        net.minecraft.client.gui.Gui.drawModalRectWithCustomSizedTexture(0, 0, 0, 0, (int) width, (int) height, (int) width, (int) height);
        GL11.glPopMatrix();
        restoreGuiRenderState(depthEnabled, blendEnabled, depthMask);
    }
